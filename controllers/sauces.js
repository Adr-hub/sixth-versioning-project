...
exports.postSauces = (req, res, next) => {
    if (JSON.parse(req.body.sauce).userId === req.authorize.userId) {
        ...
        new sauceSchema(sauceData).save()
            .then(() => {
                res.status(201).json({ message: 'Successful submission !' });
            })
            .catch((error) => {
                res.status(500).json(error);
            });
        ...
        .then((data) => {
            ...
            if (!data.usersLiked.includes(req.authorize.userId)) {
                ...
                if (req.body.like === 1) {
                    ...
                    sauceSchema.updateOne({ _id: data._id }, { likes: data.likes + 1, $push: { usersLiked: req.authorize.userId } }).then(() => {
                ...
                if (pushedUser === req.authorize.userId) {
                    ...
                    if (data.userId !== req.authorize.userId) {
                        sauceSchema.updateOne({ _id: data._id }, { likes: data.likes - 1, $pull: { usersLiked: pushedUser } })
                            .catch((error) => {
                                res.status(500).json(error);
                            })
        ...
        .then((data) => {
            ...
            if (!data.usersDisliked.includes(req.authorize.userId)) {
                ...
                if (req.body.like === -1) {
                    ...
                    sauceSchema.updateOne({ _id: data._id }, { dislikes: data.dislikes + 1, $push: { usersDisliked: req.authorize.userId } }).then(() => {
                ...
                if (pushedUser === req.authorize.userId) {
                    ...
                    if (data.userId !== req.authorize.userId) {
                        ...
                        sauceSchema.updateOne({ _id: data._id }, { dislikes: data.dislikes - 1, $pull: { usersDisliked: pushedUser } })
                            .catch((error) => {
                                res.status(500).json(error);
                            })
                        res.status(200).json({ message: 'Successful cancelation !' });
